...
const network = __ENV.NETWORK ?? "mendoza";

const BASE_URL = `https://explorer.${network}.hoodi.arkiv.network`;
const READ_REQUESTS_PER_ITERATION = Number(
  __ENV.READ_REQUESTS_PER_ITERATION ?? 3
);
const MAX_BLOCK_HEIGHT = Number(__ENV.MAX_BLOCK_HEIGHT ?? 29_000);
